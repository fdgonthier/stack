...
#define SHIM_NAME   	    "shim-eth-rina"
...
struct ipcp_instance_data {
        ...
        struct eth_rina_info * info;
        struct packet_type *   eth_rina_packet_type;
        struct net_device *    dev;
        struct flow_spec *     fspec;

        /* The IPC Process using the shim-eth-rina */
...
/* Needed for eth_rina_rcv function */
...
static ssize_t eth_rina_ipcp_attr_show(struct robject *        robj,
                                       ...
                                       char *                  buf)
{
	...
	if (strcmp(robject_attr_name(attr), "type") == 0)
		return sprintf(buf, "shim-eth-rina\n");
...
RINA_SYSFS_OPS(eth_rina_ipcp);
RINA_ATTRS(eth_rina_ipcp, name, type, dif, address, iface, tx_busy);
RINA_KTYPE(eth_rina_ipcp);
                                   ...
                                   struct shim_eth_flow *      flow)
{
	ASSERT(data);
        ...
        LOG_DBG("Shim eth-rina unbinded port: %u", flow->port_id);
        if (flow_destroy(data, flow)) {
                LOG_ERR("Failed to destroy shim eth-rina flow");
                return -1;
        }
...
static int eth_rina_unbind_user_ipcp(struct ipcp_instance_data * data,
                                     port_id_t                   id)
{
        struct shim_eth_flow * flow;
...
static int
eth_rina_flow_allocate_request(struct ipcp_instance_data * data,
                               struct ipcp_instance *      user_ipcp,
...
static int
eth_rina_flow_allocate_response(struct ipcp_instance_data * data,
                                struct ipcp_instance *      user_ipcp,
                                port_id_t                   port_id,
                                int                         result)
{
...
static int eth_rina_flow_deallocate(struct ipcp_instance_data * data,
                                    port_id_t                   id)
{
        struct shim_eth_flow * flow;

        LOG_DBG("eth_rina_flow_deallocate");
...
static int eth_rina_application_register(struct ipcp_instance_data * data,
					 ...
					 const struct name *         daf_name)
{
	...
	if (!name) {
        ...
        if (data->app_name) {
                tmp = name_tostring(data->app_name);
        ...
        tmp = name_tostring(name);
        LOG_DBG("eth_rina_application_register[name = %s]", tmp);
        rkfree(tmp);
...
static int eth_rina_application_unregister(struct ipcp_instance_data * data,
                                           const struct name *         name)
{
        LOG_DBG("eth_rina_application_unregister");
	...
	if (!name) {
        ...
        if (!data->app_name) {
                LOG_ERR("Shim eth-rina has no application registered");
                return -1;
        }
...
static void enable_write_all(struct net_device * dev)
{
	...
	ASSERT(dev);
        ...
        list_for_each_entry(pos, &(eth_rina_data.instances), list) {
                if (pos->dev == dev)
                	enable_all_port_ids(pos);
        }
}

static void eth_rina_skb_destructor(struct sk_buff *skb)
{
	struct ipcp_instance_data *data =
...
static int eth_rina_du_write(struct ipcp_instance_data * data,
                             port_id_t                   id,
                             struct du *                 du,
                             bool                        blocking)
{
        ...
        LOG_DBG("eth_rina_du_write[size = %ld]", du_len(du));
	...
	bup_skb->destructor = &eth_rina_skb_destructor;
...
static int eth_rina_rcv_worker(void * o)
{
        struct ipcp_instance_data *     data;
	...
	ASSERT(o);
        ...
        if (!user_ipcp || !ipcp) {
                ...
                if (flow_destroy(data, flow))
                        LOG_ERR("Problems destroying shim-eth-rina "
                                "flow");
                return -1;
        }
...
static int eth_rina_recv_process_packet(struct sk_buff *    skb,
					struct net_device * dev)
{
        struct ethhdr *                 mh;
        unsigned char *                 saddr;
	...
	if (!du) {
        ...
        if (!flow) {
                ...
                wdata->flow = flow;
                wdata->data = data;
                item  = rwq_work_create_ni(eth_rina_rcv_worker, wdata);

                rwq_work_post(rcv_wq, item);

                LOG_DBG("eth_rina_recv_process_packet added work");
        } else {
                gha_destroy(ghaddr);
                ...
                LOG_DBG("eth_rina_recv_process_packet ends ...");
        }

        return 0;
}

static int eth_rina_rcv(struct sk_buff *     skb,
                        struct net_device *  dev,
                        ...
                        struct net_device *  orig_dev) /* not used */
{
	...
	LOG_DBG("eth_rina_rcv started, skb received");
        ...
        if (eth_rina_recv_process_packet(skb, dev))
                LOG_DBG("Failed to process packet");

        LOG_DBG("eth_rina_rcv ends");
        return 0;
};

static int eth_rina_assign_to_dif(struct ipcp_instance_data * data,
				  ...
				  struct dif_config * config)
{
	...
	if (!data) {
        ...
        nm = name_tostring(data->dif_name);
        LOG_DBG("eth_rina_assign_to_dif[dif_name = %s, type = %s]", nm, type);
        rkfree(nm);
        ...
        data->eth_rina_packet_type->type = cpu_to_be16(ETH_P_RINA);
        data->eth_rina_packet_type->func = eth_rina_rcv;
	...
	read_unlock(&dev_base_lock);
        ...
        if (!mapping) {
                /* FIXME: Unlikely to happen but please let's log a
        ...
        data->eth_rina_packet_type->dev = data->dev;
        dev_add_pack(data->eth_rina_packet_type);

        LOG_DBG("Configured shim eth-rina IPC process, using interface %s",
                complete_interface);
...
static int eth_rina_update_dif_config(struct ipcp_instance_data * data,
                                      const struct dif_config *   new_config)
{
	...
	dev_remove_pack(data->eth_rina_packet_type);
...
static const struct name * eth_rina_ipcp_name(struct ipcp_instance_data * data)
{
        ASSERT(data);
...
static const struct name * eth_rina_dif_name(struct ipcp_instance_data * data)
{
	if (!data) {
		LOG_ERR("Bogus data passed, bailing out");
		return NULL;
...
static size_t eth_rina_max_sdu_size(struct ipcp_instance_data * data)
{
	if (!data) {
		LOG_ERR("Bogus data passed, bailing out");
		return 0;
...
ipc_process_id_t eth_rina_ipcp_id(struct ipcp_instance_data * data)
{
	ASSERT(data);
	return data->id;
}

static int eth_rina_query_rib(struct ipcp_instance_data * data,
...
static struct ipcp_instance_ops eth_rina_instance_ops = {
        .flow_allocate_request     = eth_rina_flow_allocate_request,
        .flow_allocate_response    = eth_rina_flow_allocate_response,
        .flow_deallocate           = eth_rina_flow_deallocate,
        .flow_prebind              = NULL,
        .flow_binding_ipcp         = NULL,
        .flow_unbinding_ipcp       = NULL,
        .flow_unbinding_user_ipcp  = eth_rina_unbind_user_ipcp,
	.nm1_flow_state_change	   = NULL,

        .application_register      = eth_rina_application_register,
        .application_unregister    = eth_rina_application_unregister,

        .assign_to_dif             = eth_rina_assign_to_dif,
        .update_dif_config         = eth_rina_update_dif_config,
	...
	.pff_modify		   = NULL,
        ...
        .query_rib		   = eth_rina_query_rib,

        .ipcp_name                 = eth_rina_ipcp_name,
        .dif_name                  = eth_rina_dif_name,
	.ipcp_id		   = eth_rina_ipcp_id,
	...
	.address_change            = NULL,
        .dif_name		   = eth_rina_dif_name,
	.max_sdu_size		   = eth_rina_max_sdu_size
};
...
static int eth_rina_netdev_notify(struct notifier_block *nb,
				  unsigned long event,
				  void *opaque)
{
	struct net_device *dev;
...
static int eth_rina_init(struct ipcp_factory_data * data)
{
	ASSERT(data == &eth_rina_data);
	...
	data->ntfy.notifier_call = eth_rina_netdev_notify;
...
static int eth_rina_fini(struct ipcp_factory_data * data)
{
...
static void inst_cleanup(struct ipcp_instance * inst)
{
        ...
        if (inst->data) {
                ...
                if (inst->data->eth_rina_packet_type)
                        rkfree(inst->data->eth_rina_packet_type);
...
static struct ipcp_instance * eth_rina_create(struct ipcp_factory_data * data,
                                              const struct name *        name,
                                              ipc_process_id_t           id,
                                              uint_t			 us_nl_port)
{
	...
	ASSERT(name);
        ...
        /* fill it properly */
        inst->ops  = &eth_rina_instance_ops;
	...
	if (robject_rset_init_and_add(&inst->robj,
        ...
        inst->data->eth_rina_packet_type =
	...
	inst->data->tx_busy = 0;
        ...
        LOG_DBG("KFA instance %pK bound to the shim eth-rina", inst->data->kfa);
...
static int eth_rina_destroy(struct ipcp_factory_data * data,
                            struct ipcp_instance *     instance)
{
        struct interface_data_mapping * mapping;
        ...
        LOG_DBG("Looking for the eth-rina instance to destroy");
        ...
        list_for_each_entry_safe(pos, next, &data->instances, list) {
                if (pos->id == instance->data->id) {
                        ...
                        if (pos->eth_rina_packet_type->dev)
                                __dev_remove_pack(pos->eth_rina_packet_type);
                         ...
                         */
                        if (pos->eth_rina_packet_type)
                                rkfree(pos->eth_rina_packet_type);

                        rkfree(pos);
                        rkfree(instance);

                        LOG_DBG("eth-rina instance destroyed, returning");
...
static struct ipcp_factory_ops eth_rina_ops = {
        .init      = eth_rina_init,
        .fini      = eth_rina_fini,
        .create    = eth_rina_create,
        .destroy   = eth_rina_destroy,
};

static struct ipcp_factory * shim_eth_rina = NULL;
...
static int __init mod_init(void)
{
        ...
        shim_eth_rina = kipcm_ipcp_factory_register(default_kipcm,
                                           	    SHIM_NAME,
                                          	    &eth_rina_data,
						    &eth_rina_ops);
        if (!shim_eth_rina)
...
{
        ASSERT(shim_eth_rina);

        flush_workqueue(rcv_wq);
        destroy_workqueue(rcv_wq);

        kipcm_ipcp_factory_unregister(default_kipcm, shim_eth_rina);
